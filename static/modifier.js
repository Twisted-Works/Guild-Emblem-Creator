...
async function changeImageColor(img, red, green, blue) {
  var rgbks = await this.generateRGBKs(img)

  return this.generateTintImage(img, rgbks, red, green, blue)
}

async function updateBaseImageColors(images, colors) {
  for (let key in images) {
    var colorKey = key + "Color"
    if (colors[colorKey] !== undefined) {
      var rgb = await this.hexToRGB(colors[colorKey])
